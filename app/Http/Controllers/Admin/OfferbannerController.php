...
class OfferbannerController extends Controller
{
    ...
    {
        $banner=DB::table('offer_banner')->select('*')->get();
     
        return view('admin.offer_banner.index', compact('banner'));
    }
    
    public function delete_offerbanner($id)
    {
        $affectedRows = Offerbanner::where('banner_id', $id)->delete();
        return redirect()->back()->with('info', "Record delete successfully !");
    }

    public function add_offerbanner(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                return redirect('admin/offerbanner/add')->withInput()->withErrors($validator);
            }
            ...
            return view('admin.offer_banner.add');
        }
    }

    public function edit_offerbanner(Request $request, $id)
    {
        if ($request->isMethod('post')) {
            // $messages = [
           ...
           $rules = [
            ...
            $banner=Offerbanner::where('banner_id', $id)->first();
            return view('admin.offer_banner.edit', compact('banner'));
        }
    }
}
