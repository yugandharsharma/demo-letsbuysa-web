...
@section('content')
    ...
    <div class="container">
      <nav class="breadcrumb-nav">
        <ul class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="#"><?php echo  __('messages.bank_account_and_payment') ?></a></li>
        </ul>
      </nav>
    </div>
  </section>
  <section class="in-banner-line">
           <div class="container">
              <div class="help-bx">
                @if((\App::getLocale() == 'en'))
                <h5>{{$bankaccount->title_en}}</h5>
                @else
                <h5>{{$bankaccount->title_ar}}</h5>
                @endif
          ...
          <div class="container">
            <div class="support-tabs pt-0">
              <ul class="tab-text-in pt-0">
                @if((\App::getLocale() == 'en'))
                  {!!$bankaccount->description_en!!}
                @else
                  {!!$bankaccount->description_ar!!}
                @endif
              </ul>
            </div>
          </div>
