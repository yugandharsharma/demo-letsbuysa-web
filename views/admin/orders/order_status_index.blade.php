  ...
  <div class="content-wrapper">
    <div class="container-fluid">
     ...
     <div class="row pt-2 pb-2">
        <div class="col-sm-9">
        <h4 class="page-title">Order Status Tables</h4>
        <ol class="breadcrumb">
       ...
       <!-- <div class="col-sm-3">
       <a href="{{url('admin/order_status/add')}}">
                ...
                <tbody>
                  @if($orderstatus)
                    @php
                     $i=1;
                     @endphp
                    @foreach($orderstatus as $record)
                    <tr>
                        <td>{{$i}}</td>
                        <td>{{$record->status_name_en}}</td>
                        <td>{{$record->status_name_ar}}</td>
                        <td>
                          <!-- <a href="{{ url('/')}}/admin/order_status/delete/{{$record->id}}">
                          <button onclick="return confirm('are you want to delete this record');" type="button" class="btn btn-danger waves-effect waves-light m-1"> <i class="fa fa-trash-o"></i> </button></a> -->
                           @if(auth()->user()->role === 1)
                          <a href="{{ url('/')}}/admin/order_status/edit/{{$record->id}}">
                          ...
                          @php $permission = permission(); @endphp
                              ...
                              @if(auth()->user()->role === 3 && isset($permission['order_status']) && $permission['order_status']['add'] === 1)
                                  <a href="{{ url('/')}}/admin/order_status/edit/{{$record->id}}">
                                      <button type="button" class="btn btn-success waves-effect waves-light m-1"> <i class="fa fa-pencil"></i> </button></a>
                              @endif
