  ...
  <div class="content-wrapper">
    <div class="container-fluid">
     ...
     <div class="row pt-2 pb-2">
        <div class="col-sm-9">
        <h4 class="page-title">Offer Banner content Tables</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">Dashboard</a></li>
            <li class="breadcrumb-item active" aria-current="page">Offer Banner content List</li>
         </ol>
     </div>
     <div class="col-sm-3">
       <a href="{{url('admin/offerbanner/add')}}">
             ...
             <table id="list" class="table table-bordered">
                ...
                <tbody>
                  @if($banner)
                    ...
                    <tr>
                        <td>{{$i}}</td>
                        <td><img src="{{ url('/') }}/public/images/banners/{{$record->image_en}}"  alt="User Image" width="150"></td>
                        <td><img src="{{ url('/') }}/public/images/banners/{{$record->image_ar}}"  alt="User Image" width="150"></td>
                        <td>@if($record->status == 1)Enabled @else Disabled @endif</td>
                        <td>
                            @if(auth()->user()->role === 1)
                          <a href="{{ url('/')}}/admin/offerbanner/delete/{{$record->banner_id}}">
                          ...
                          <a href="{{ url('/')}}/admin/offerbanner/edit/{{$record->banner_id}}">
                          <button type="button" class="btn btn-success waves-effect waves-light m-1"> <i class="fa fa-pencil"></i> </button></a>
                            @endif
                                @php $permission = permission(); @endphp
                                    ...
                                    @if(auth()->user()->role === 3 && isset($permission['offer_banners']) && $permission['offer_banners']['delete'] === 1)
                                        <a href="{{ url('/')}}/admin/offerbanner/delete/{{$record->banner_id}}">
                                            <button onclick="return confirm('are you want to delete this record');" type="button" class="btn btn-danger waves-effect waves-light m-1"> <i class="fa fa-trash-o"></i> </button></a>
                                    @endif
                                    @if(auth()->user()->role === 3 && isset($permission['offer_banners']) && $permission['offer_banners']['edit'] === 1)
                                            <a href="{{ url('/')}}/admin/offerbanner/edit/{{$record->banner_id}}">
                                                <button type="button" class="btn btn-success waves-effect waves-light m-1"> <i class="fa fa-pencil"></i> </button></a>
                                    @endif
